// =====================================================================
// 데모 캐스팅 — mock 만으로 8 초 슬롯 추출 + 군중 생성 애니메이션.
// /demo/casting → 8s 뒤 /demo/live 로 자동 navigate.
// API 호출 없음. 모든 데이터는 lm 의 SEED / ANCHORS.
// =====================================================================
...
export default function CastingDemoMock() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setT(newT);
      if (newT < 1) rafRef.current = requestAnimationFrame(tick);
    };
  ...
  const extractedIds = useMemo(
    () => anchorTimes.map((at, i) => (t >= at ? anchors[i].id : null)).filter(Boolean) as string[],
  ...
  const handleEnter = () => navigate('/demo/live');
      ...
      <div className="lm-cast__pad">
        <header className="lm-cast__head">
          <div className="lm-cast__head-left">
            <div className="lm-cast__head-eyebrow">Phase 4 · 페르소나 생성 (데모)</div>
          ...
          <div className="lm-cast__head-actions">
            <Button kind="primary" disabled={!done} onClick={handleEnter} trailing={<ArrowGlyph dir="right" />}>
